...
void cb_func(const Response& res, Buffer* buffer, void* arg){
    char* mm = (char*)arg;
...
void cb_func2(const Response& res, Buffer* buffer, void* arg){
...
static void test_libchunk(void *arg1, void *arg2){
    ...
    cmd_client_stub->submit(*rdma_work_request_write_inline, peer_io_addr, &cb_func2, nullptr, nullptr); //**这里第三个参数写nullptr但是在内部调用时因为时inline_read，会自动将参数定位到recv的rdma buffer上
    ...
    cmd_client_stub->submit(*rdma_work_request_read_inline, peer_io_addr, &cb_func, nullptr, nullptr); //**这里第三个参数写nullptr但是在内部调用时因为时inline_read，会自动将参数定位到recv的rdma buffer上
